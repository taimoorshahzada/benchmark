...
export default function ProcessesList({ processes, walkthroughs }: any) {
  ...
  useEffect(() => {
    ...
    const checkScrollPos = () => {
      ...
      processElements.forEach((process, index) => {
        ...
        if (prevScrollHeight && processContainer && processContainer.scrollTop < prevScrollHeight) {
        ...
        else if (prevScrollHeight && processContainer && processContainer.scrollTop > prevScrollHeight) {
          if (processRect.bottom < window.innerHeight && processRect.bottom > window.innerHeight - 300) {
            if (titleIndex) {
              titleIndex.innerText = index < 10 ? `0${index + 1}` : `${index}`
            }
            if (title) {
  ...
  return (
    <div className="flex">
      <div className="w-1/2 bg-black relative">
        <div className="sticky top-0 h-screen">
          <div className="h-full flex flex-col justify-end pl-3 pb-10">
            <h1 className="font-medium text-white text-lg lg:text-xl xl:text-2xl mb-medium" id="process-title-index">
              01
            </h1>
            <h1 className="font-medium break-words text-white text-lg lg:text-xl xl:text-2xl" id="process-title">
      ...
      <div className="w-1/2">
        <h1 className="font-medium text-lg lg:text-xl xl:text-2xl mb-[90px] px-5">
          Our <br /> Process
        </h1>
        {processes.map((process: any, index: number) => (
          <div key={index} className={`w-full process ${index == processes.length - 1 ? "" : "px-5"}`}>
            {index == processes.length - 1 ? <div className="bg-gradient-to-b to-black from-white h-[300px] w-full mt-large"></div>: ""}
            <div className={`${index == processes.length - 1 ? "pt-medium bg-black text-white px-5" : "mt-large border-grey border-dashed border-t border-1"} flex flex-col`}>
              {index == processes.length - 1 ? "" :
                <div className="mt-[10px] mb-10 text-xs-medium">
                  { index < 10 ? `0${index + 1}` : index}
                </div>
              }
              <Image className="w-full h-auto mb-[10px]" src={process.hero_image.imageUrl} alt="Process Image" width={1000} height={1000} />
        ...
        <div className="pt-large bg-black grid grid-cols-2 gap-x-[2px] text-xs-medium text-white gap-y-[60px] pb-[180px]">
          <div className="mb-5 px-5">
            Virtual walkthroughs
          </div>
          {walkthroughs.props.walkthroughs.map((walkthrough: any, index: number) => (
            <div key={index} className={`${index % 2 == 0 ? "col-start-1 ml-5 pr-[9px]": "col-start-2 pl-[9px] mr-5"} flex flex-col border-grey border-dashed border-t border-1`}>
              ...
              <iframe allowFullScreen src={walkthrough.url} title={walkthrough.title}></iframe>
            </div>
          ))}
          {walkthroughs.props.walkthroughs.length % 2 != 0 ? <div className="border-grey border-dashed border-t border-1 mr-5"></div> : ""}
        </div>
        <div className="flex flex-col mx-5">
          <h1 className="font-medium text-lg lg:text-xl xl:text-2xl mb-large px-5">
            Get In Touch
          </h1>
